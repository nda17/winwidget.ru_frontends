...
compose config --format json 2>/dev/null | node_validate '
	...
	for(const [app,port] of Object.entries(apps)){
		...
		if(a.FRONTEND_APP!==app || !/^[a-f0-9]{40}$/.test(a.APP_REVISION) || s.image!==`winwidget-${app}:git-${a.APP_REVISION}` || a.NEXT_PUBLIC_WINCRM_ENABLED!=="true" || a.NEXT_PUBLIC_WINCRM_BILLING_ENABLED!==billingUi)fail();
