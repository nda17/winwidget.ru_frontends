...
| ------------------ | --------------------------------------------------------------------------------- | -------------- |
| `apps/landing`     | Прежний лендинг виджетов: `winwidget.ru`                                          | 3000           |
...
Публичная структура: `/` — прежний полный лендинг виджетов; меню «Главная»
и активная «CRM» → `https://crm.winwidget.ru`. До запуска CRM на её домене
открывается страница «Скоро». Якоря `/#tools`, `/#pricing`, `/#faq` работают
на главной без перенаправлений. Экспериментальные `/products/widgets` и
`/products/crm` удалены по решению владельца, без совместимых маршрутов.
Рабочие URL кабинета, auth, оплаты и общей админки не меняются;
переключатель продуктов в рабочих приложениях не активирует Trial и
не заменяет серверную проверку независимых подписок.

В «Контенте» ADMIN/DEV редактируют главную страницу, демо-виджеты, Footer
и SEO; raw head/body остаются DEV-only. Сохранённые `ecosystem` и `crmProduct`
совместимо передаются в Platform JSON, но больше не имеют публичных страниц
и редакторов. Откат представления не удаляет эти данные и не меняет цены или
release gates. Публичный контент обновляется с текущим cache TTL до 60 секунд.
Sitemap сохраняет явные отключения и пользовательские записи; удалённые
продуктовые страницы не публикуются даже при наличии старых записей в CMS.
